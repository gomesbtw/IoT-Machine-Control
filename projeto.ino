...
Adafruit_MQTT_Publish currentSensorFeed = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/vibracao"); // Feed para enviar leituras do sensor
Adafruit_MQTT_Subscribe relayControlFeed = Adafruit_MQTT_Subscribe(&mqtt, AIO_USERNAME "/feeds/maq_comando"); // Feed para controlar um relé (não usado diretamente)
...
void loop() {
  ...
  if (mqtt.readSubscription()) {
    String lastRead = String((char*)relayControlFeed.lastread);
    mqtt.processPackets(10000);
    Serial.println(lastRead);
    if (lastRead == "21" and chave==0 ) {
      // Ligar LED (Exemplo)
      chave = 1;
  
      digitalWrite(ledPin, HIGH);
    } else if (lastRead == "21" and chave ==1 ) {
      // Desligar LED (Exemplo)
      chave = 0;
  ...
  if (leituraSensor == HIGH and estadoLED ==HIGH) { // Detecta atividade no sensor
   
      Serial.println("Vibração detectada");
      unsigned long startTime = millis(); //inicia a contagem
        ...
        unsigned long endTime = millis(); // Marca o tempo após o envio
    
        unsigned long duration = endTime - startTime; // Calcula a duração do envio
        Serial.println("Tempo para enviar: " + String(duration) + " ms"); // Exibe o tempo no monitor serial
